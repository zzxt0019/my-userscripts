...
(function () {
    ...
    function hide(list) {
        ...
        if (!list) {
            list = document.querySelectorAll('img[src]:not([zzxt]):not([src="about:blank"])');
        }
        for (let i = 0; i < list.length; i++) {
            let count0 = count++;
            let img = list[i];
            if (ZhiHu(img)) {
                img.setAttribute('zzxt', String(count0));
                let button = document.createElement('button');
                button.setAttribute('zzxt', String(count0));
                ...
                button.onclick = () => {
                    button.setAttribute('display', button.getAttribute('display') === 'none' ? '' : 'none');
                    button.innerHTML = button.getAttribute('display') === 'none' ? styles.buttonSvgToSee : styles.buttonSvgToHide;
                    document.querySelector('img[zzxt="' + count0 + '"]').style.display = button.getAttribute('display');
                };
     ...
     */
    function check() {
        let list = document.querySelectorAll('button[zzxt][display=none]');
        for (let i = 0; i < list.length; i++) {
            let button = list[i];
            let img = document.querySelector('img[zzxt="' + button.getAttribute('zzxt') + '"]');
            if (img && img.style.display !== button.getAttribute('display')) {
                img.style.display = button.getAttribute('display');
            }
        }
    }
