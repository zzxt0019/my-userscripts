...
(function () {
    ...
    const NoneDisplayKey = 'zzxt0019-hide-none-display';
            ...
            if (ZhiHu(img)) {
                ...
                button.onclick = () => {
                    if (button.getAttribute(DisplayKey) === 'none') {
                        ...
                        document.querySelector(`[${UserscriptImgId}="${count0}"]`).classList.add(NoneDisplayKey);
                    } else {
                        button.setAttribute(DisplayKey, 'none');
                        button.innerHTML = styles.buttonSvgToSee;
                        document.querySelector(`[${UserscriptImgId}="${count0}"]`).classList.add(DisplayKey);
                        document.querySelector(`[${UserscriptImgId}="${count0}"]`).classList.remove(NoneDisplayKey);
                    }
                };
